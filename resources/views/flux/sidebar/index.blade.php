...
@php
    ...
    $classes = Flux::classes('[grid-area:sidebar]')
        ...
        ->add(
            'data-flux-sidebar-collapsed-desktop:cursor-e-resize rtl:data-flux-sidebar-collapsed-desktop:cursor-w-resize',
        );
...
<ui-sidebar {{ $attributes->class($classes) }}
    @if ($collapsible) collapsible="{{ $collapsible === 'mobile' ? 'mobile' : 'true' }}" @endif
    @if ($stashable) stashable @endif @if ($sticky) sticky @endif x-data
    data-flux-sidebar-cloak data-flux-sidebar>
    {{ $slot }}
</ui-sidebar>
